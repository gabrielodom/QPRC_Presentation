...
Under the Single-State model, the pairs of AD-PCA and MSAD-PCA process monitoring statistics perform similarly, so we do not expect significant loss of power when using the MSAD-PCA procedure.
...
## False Alarm Rates
We increased the flags to trigger an alarm from 3 to 5 for the real case due to the strong serial autocorrelation. Note that these are all greater than or equal to the set threshold level of $\alpha = 0.1\%$.
...
- Discussed the simulation study results.
- Demonstrated the utility of MSAD-PCA when applied to a real data set.
